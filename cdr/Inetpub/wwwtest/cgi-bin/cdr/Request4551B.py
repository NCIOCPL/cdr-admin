#----------------------------------------------------------------------
#
# $Id: Request4551B.py,v 1.1 2009-04-29 00:14:07 venglisc Exp $
#
...
# -------------------------------------------------------------
# Read the spreadsheet from Kim (CDR-list)
# -------------------------------------------------------------
def readCdrList(filename = 'd:/cdr/tmp/CDR_Protocols.xls'):
    cdrBook = ExcelReader.Workbook(filename)
    sheet = cdrBook[6]
    headerRow = 1
    rownum = 0
    cdrProtocols = {}
    ...
    for row in sheet.rows:
        ...
        nctId      = row[0]
        cdrId      = row[1]
        cdrProtocols[nctId.val] = {u'CDRID':str(cdrId)}
...
# -------------------------------------------------------------
# Read the spreadsheet from NLM (NCT-list)
# -------------------------------------------------------------
def readNctList(filename = 'd:/cdr/tmp/NLM_Protocols.xls'):
    nctBook = ExcelReader.Workbook(filename)
    sheet = nctBook[0]
    headerRow = 9
    rownum = 0
    hasOutcomes     = []
    ...
    for row in sheet.rows:
        i += 1
        if i > 5:
            if row[4]: hasOutcomes.append(row[4].val)
            if row[5]: hasStartDate.append(row[5].val)
            if row[6]: hasEnrollment.append(row[6].val)
            if row[7]: hasNumArms.append(row[7].val)
            if row[8]: hasRespParty.append(row[8].val)
            if row[9]: hasSafety.append(row[9].val)
            if row[10]: hasFda.append(row[10].val)
            if row[11]: hasEndDate.append(row[11].val)
            if row[12]: hasEndDatePrime.append(row[12].val)

        #if i > 10:
        #    return (hasOutcomes, hasStartDate, hasEnrollment, hasNumArms, 
        #        hasRespParty, hasSafety, hasFda, hasEndDate, hasEndDatePrime)
...
# -------------------------------------------------------------
quickTitle = "Compare CDR and NCT Report"
t = time.strftime("%Y%m%d%H%M%S")
REPORTS_BASE = 'd:/cdr/tmp'
#REPORTS_BASE = 'm:/cdr/tmp'
name = '/CompareCdrNct-%s.xml' % t
cdrName = '/test_cdr.xls'
nctName = '/test_nct.xls'
fullname = REPORTS_BASE + name

# ----------------------------------------------------------------------
# First Step:
# We need to read the content of the Spreadsheets provided
# ----------------------------------------------------------------------
cdrList = readCdrList(filename = REPORTS_BASE + cdrName)
nctList = readNctList(filename = REPORTS_BASE + nctName)
...
# Split the NCT list into the distinct columns to search 
# ------------------------------------------------------
hasOutcomes     = nctList[0]
hasStartDate    = nctList[1]
hasEnrollment   = nctList[2]
hasNumArms      = nctList[3]
hasRespParty    = nctList[4]
hasSafety       = nctList[5]
hasFda          = nctList[6]
hasEndDate      = nctList[7]
hasEndDatePrime = nctList[8]
...
exRow.addCell(3, 'Has Outcomes')
exRow.addCell(4, 'Has Start Date')
exRow.addCell(5, 'Has Enrollment')
exRow.addCell(6, 'Has Number of Arms')
exRow.addCell(7, 'Has Resp Party')
exRow.addCell(8, 'Has Safety Issue')
exRow.addCell(9, 'Has FDA Regulated')
exRow.addCell(10, 'Has End Date')
exRow.addCell(11, 'Has End Date Prime')

# Add the protocol data one record at a time beginning after 
# the header row
# ----------------------------------------------------------
rowNum = 1
for row in cdrList.keys():
    if cdrList[row][u'CDRID'] != 'CDR ID':
        rowNum += 1
        exRow = ws.addRow(rowNum, style1, 40)
        exRow.addCell(1, row)
        exRow.addCell(2, cdrList[row][u'CDRID'])
        if row in hasOutcomes:
            exRow.addCell(3, 'No')
        if row in hasStartDate:
            exRow.addCell(4, 'No')
            #print 'No StartDate: %s' % row
        if row in hasEnrollment:
            exRow.addCell(5, 'No')
            #print 'No Enrollment: %s' % row
        if row in hasNumArms:
            exRow.addCell(6, 'No')
            #print 'No NumArms: %s' % row
        if row in hasRespParty:
            exRow.addCell(7, 'No')
            #print 'No RespParty: %s' % row
        if row in hasSafety:
            exRow.addCell(8, 'No')
            #print 'No Safety: %s' % row
        if row in hasFda:
            exRow.addCell(9, 'No')
            #print 'No hasFda: %s' % row
        if row in hasEndDate:
            exRow.addCell(10, 'No')
            #print 'No EndDate: %s' % row
        if row in hasEndDatePrime:
            exRow.addCell(11, 'No')
            #print 'No EndDatePrime: %s' % row

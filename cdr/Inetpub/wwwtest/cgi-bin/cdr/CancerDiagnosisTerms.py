...
try:
    cursor.execute("""\
        SELECT DISTINCT doc_id, 0 parent_id
                   INTO #cancer_diagnosis_semantic_types
                   FROM query_term
                  WHERE path = '/Term/PreferredName'
                    AND value = 'Cancer diagnosis'""")
    conn.commit()
    done = 0
    while not done:
        cursor.execute("""\
            INSERT INTO #cancer_diagnosis_semantic_types(doc_id, parent_id)
                 SELECT q.doc_id, q.int_val
                   FROM query_term q
                   JOIN #cancer_diagnosis_semantic_types cdst
                     ON cdst.doc_id = q.int_val
                  WHERE q.doc_id NOT IN
                        (
                            SELECT doc_id
                              FROM #cancer_diagnosis_semantic_types
                        )
        ...
        conn.commit()
    cursor.execute("""\
        SELECT cdst.doc_id, cdst.parent_id, pname.value
          FROM #cancer_diagnosis_semantic_types cdst
          JOIN query_term pname
            ON pname.doc_id = cdst.doc_id
    ...
    cursor.execute("""\
        SELECT cdst.doc_id SemanticTypeId, 
               cdt.doc_id IndexTermId, 
               pname.value IndexTermName
          INTO #index_terms
          FROM #cancer_diagnosis_semantic_types cdst
          JOIN query_term cdt
            ON cdt.int_val = cdst.doc_id
          JOIN query_term pname
            ON pname.doc_id = cdt.doc_id
         WHERE cdt.path = '/Term/SemanticType/@cdr:ref'
...
<html>
 <head>
  <title>CDR Cancer Diagnosis Index Terms</title>
  <style type = 'text/css'>
 ...
 <body>
  <h1>CDR Cancer Diagnosis Index Terms</h1>
  <ul>

#----------------------------------------------------------------------
#
# $Id: ProtAbstractUpdateMailer.py,v 1.2 2002-02-21 22:34:01 bkline Exp $
#
# Request form for Update Protocol Abstract Mailer.
#
...
section    = "Protocol Abstract Update Mailer"
SUBMENU    = 'Mailer Menu'
buttons    = ["Submit", SUBMENU, cdrcgi.MAINMENU, "Log Out"]
script     = 'ProtAbstractUpdateMailer.py'
header     = cdrcgi.header(title, title, section, script, buttons)
subsetName = 'Update Protocol Abstract Verification Mailers'
initial    = 'Initial Protocol Abstract Verification Mailers'
...
if docId:
    ...
    try:
        cursor.execute("""\
            SELECT DISTINCT protocol.id, MAX(protocol.num)
                       FROM doc_version protocol
                       JOIN last_doc_publication initial_mailer
                         ON initial_mailer.doc_id = protocol.id
                       JOIN query_term prot_status
                         ON prot_status.doc_id = protocol.id
                      WHERE prot_status.value IN ('Active', 
                                                  'Approved-Not Yet Active')
                        AND prot_status.path = '%s'
                        AND initial_mailer.pub_subset = '%s'
                        AND DATEDIFF(day, initial_mailer.dt, GETDATE()) > 364
                        ...
                        AND NOT EXISTS (SELECT *
                                          FROM last_doc_publication ldp
                                         WHERE protocol.id = ldp.doc_id
                                           AND ldp.pub_subset = '%s'
                                           AND 365 > DATEDIFF(day, 
                                                              ldp.dt, 
                                                              GETDATE()))
...
result = cdrpub.initNewJob(ctrlDocId, subsetName, session, docList, [],
                           email)
if type(result) == type(""):
    cdrcgi.bail(result)

...
class Board:
    ...
    def __init__(self, control, doc_id):
        ...
        if self.language != "English":
            cols.append("o.value")
        query = cdrdb.Query("query_term t", *cols).unique()
        ...
        if self.language == "English":
            ...
            query.join("query_term o", "o.doc_id = e.int_val")
    ...
    def to_table(self, columns):
        ...
        opts = { "caption": u"%s (%d)" % (self.name, len(self.summaries)) }
        rows = []
        for summary in sorted(self.summaries):
            row = []
            if self.control.show_id:
                row.append(cdrcgi.Report.Cell(summary.doc_id, classes="center"))
            title = [summary.title]
            if summary.module:
                title[0] += u" (Module)"
            if summary.original_title:
                title.append(u"(%s)" % summary.original_title)
            row.append(title)
